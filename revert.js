...
async function revertGitCommit(mergeCommitSha, prTitle) {
  ...
  try {
    ...
    if (commitToRevert.parents.length < 2) {
      throw new Error('The target commit is not a merge commit and cannot be reverted this way.');
    }
    const treeSha = commitToRevert.parents[0].tree.sha;
...
async function rebuildContentFromHistory(repoBranch, targetTxnSK) {
  ...
  while (currentTxnSK && currentTxnSK !== 'ROOT') {
    ...
    summaryBlocks.unshift(Item.summary);
...
async function pushContentToGitHub(content, revertCommitSha) {
  ...
  const { data } = await octo.repos.getContent({ owner: OWNER, repo: REPO, path: TARGET_FILE, ref: BRANCH });
...
(async () => {
  try {
    ...
    const revertCommitSha = await revertGitCommit(targetTxn.mergeCommitSha, targetTxn.prTitle);
