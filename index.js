// index.js (ESM) — FULL FILE with Gemini SDK + model fallback
...
`.trim();
  ...
  for (const modelName of MODEL_CANDIDATES) {
    try {
      ...
      // 404 or unsupported → try next model silently, but log once for visibility
...
function renderChangelogEntry({ pr, commits, jira, llm }) {
  ...
  return [
    ...
    `*Merged:* ${mergedAt.slice(0,10)} • *Author:* ${author} • *Base:* ${base} ← *Head:* ${head}`,
    ``,
    `### Summary`,
    summary,
    ``,
